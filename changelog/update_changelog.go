...
func UpdateChangelogFile(filePath string) (*semver.Version, error) {
	logger.Info("Reading lines from the changelog file")
	lines, err := readLines(filePath)
	if err != nil {
		return nil, err
	}

	logger.Info("Processing the changelog content")
	version, newContent, err := processChangelog(lines)
	if err != nil {
		return nil, err
	}

	logger.Info("Writing the new content to the changelog file")
	err = writeLines(filePath, newContent)
	if err != nil {
		return nil, err
	}

	logger.Info("Changelog file updated successfully")
	return version, nil
}
...
func findLatestVersion(lines []string) (*semver.Version, error) {
	logger.Info("Starting findLatestVersion")
	...
	for _, line := range lines {
		if versionMatch := versionRegex.FindStringSubmatch(line); versionMatch != nil {
			...
			if err != nil {
				logger.Errorf("Error parsing version '%s': %v", versionMatch[1], err)
				return nil, err
			}
	...
	logger.Infof("Latest version found: %s", latestVersion)
	logger.Info("Finished findLatestVersion")
	return latestVersion, nil
}

func processChangelog(lines []string) (*semver.Version, []string, error) {
	logger.Info("Starting processChangelog")
	...
	logger.Info("Finished processChangelog")
