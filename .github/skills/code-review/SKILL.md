---
name: code-review
description: "Review pull requests and diffs in dev-toolkit — the `dev` CLI for multi-provider repository and project management — against the rios0rios0/guide standards, with extra weight on the map-based dispatch rule, parallel-execution safety, and the guarantee that user work is never lost. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `dev-toolkit`

`dev-toolkit` clones, syncs, prunes, and bootstraps repositories across GitHub, Azure DevOps, GitLab, and Codeberg, and runs language-aware build/test/lint/SAST commands. It operates on the user's whole workspace in parallel, so anything that can discard uncommitted work is the first thing to look for.
...
## What matters most in `dev-toolkit`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Provider detection is map-based, never `switch`.** That is an explicit design decision in this repository (Codeberg included). A new `switch` on provider name is a finding — see the [Mapper Design Pattern](https://github.com/rios0rios0/guide/wiki/Mapper-Design-Pattern).
- **Dirty state must survive a sync.** The WIP-branch mechanism preserves uncommitted work through temporary commits. Any path that checks out, resets, or prunes without going through it is a Critical finding — it destroys the user's work.
- **Worktree guards always win over removal rules.** The ordered guard/removal rule tables classify linked worktrees; locked, detached, dirty, and unpushed worktrees are guarded. Reordering the tables so a removal rule can fire first is a Critical finding.
- **Parallelism is bounded.** Goroutines run behind a semaphore channel; unbounded fan-out over a workspace of a hundred repositories will exhaust file descriptors and trip SSH rate limits. A new parallel path needs the same bound and a context that cancels.
- **SSH preflight runs before batch cloning.** Removing it turns one auth failure into a hundred hanging processes; any git subprocess needs `GIT_SSH_COMMAND` batch mode and a timeout.
- **System commands are platform-gated** through `runtime.GOOS` at registration time — a command that only makes sense on Linux must not appear in the Windows help.
- **SAST orchestration isolates per-tool failure.** One scanner failing must not abort the rest; the embedded default configs are the fallback when a repository has none.
- Tokens are read from `GH_TOKEN`, `AZURE_DEVOPS_EXT_PAT`, `GITLAB_TOKEN`, and `CODEBERG_TOKEN`; a token must never be interpolated into a URL that gets logged.
- All business logic accepts interfaces, and the doubles live in `internal/testutil/`.
...
- Stubs and builders for every interface live in `internal/testutil/`.
- A change to clone, sync, prune, or worktree classification needs a test for the guarded case, not only the happy path.
...
- The size of `cmd/dev-toolkit/main.go` — all CLI wiring lives there by design.
